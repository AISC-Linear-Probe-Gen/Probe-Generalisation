...
from red_teaming_probes.data.deception_apollo.data_generation import (
    ...
    get_activations_for_deception_probe,  # Reuse for concept activations (adapt for no system prompt)
)
...
# For concept-based probing (new addition)
...
# Neutral templates (negative examples, generic neutral sentences)
NEUTRAL_TEMPLATES = [
...
def generate_concept_data(
    ...
    seed: int = 42,
    split: str = "train",  # "train" uses first half, "test" uses second half
) -> List[ConceptExample]:
    ...
    if concept in CONCEPT_TEMPLATES:
        ...
        # Split templates: train gets first half, test gets second half
        mid = len(all_templates) // 2
        if split == "train":
            templates = all_templates[:mid or len(all_templates)]  # Ensure not empty
        elif split == "test":
...
if __name__ == "__main__":
    ...
    # Existing deception probe experiment (same as before)
    # Dataloaders
    ...
    for concept in TRAINING_CONCEPTS + HOLDOUT_CONCEPTS:
        ...
        test_data_triggered = generate_concept_data(concept, n_positive=50, n_negative=0, include_trigger=True, split="test", seed=123)

        # Get activations (last token)
        cham_acts_clean = get_activations(chameleon_model, tokenizer, [ex.text for ex in test_data_clean], PROBE_LAYER, pooling="last")
        cham_acts_triggered = get_activations(chameleon_model, tokenizer, [ex.text for ex in test_data_triggered], PROBE_LAYER, pooling="last")
        ...
        if concept in TRAINING_CONCEPTS:
            ...
            train_probe_cham.append(probe_cham_tpr_triggered)  # Low for evasion
        else:
            concept_results["holdout_concepts"][concept] = concept_result
            held_probe_cham.append(probe_cham_tpr_triggered)

        # Save probe for this concept
        torch.save(probe.state_dict(), OUTPUT_DIR / f"concept_{concept}_probe.pt")

    # Summary
    print("\n=== CONCEPT PROBING SUMMARY ===")
    print(f"Avg evasion TPR (training concepts): {np.mean(train_probe_cham):.4f}")
    print(f"Avg evasion TPR (holdout concepts): {np.mean(held_probe_cham):.4f}")
